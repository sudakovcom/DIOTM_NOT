...
class Discriminator_large(nn.Module):
  ...
  def __init__(self, nc = 1, ngf = 32, act=nn.LeakyReLU(0.2)):
    ...
    self.start_conv = conv2d(nc, ngf*2, 1, padding=0)
    # self.conv0 = DownConvBlock(ngf*2, ngf*2, t_emb_dim = t_emb_dim, downsample = True, act=act)
    ...
    self.final_conv = conv2d(ngf*8, ngf*8, 3, padding=1)
    self.end_linear = dense(ngf*8, 1)
...
class Discriminator_largest(nn.Module):
  """A time-dependent discriminator for large images (CelebA, LSUN)."""
